...
When `Google Cloud Run` is selected, step 2 shows Cloud Run preflight, Google Cloud login, project/region settings, dry-run, and deploy controls. The default region is `europe-west2` (UK London). The region field has quick picks for `europe-west2` (UK London), `us-central1`, `asia-east2` (Hong Kong), and `asia-southeast1` (Singapore), while still accepting another valid Cloud Run region ID. The default poll schedule is `* * * * *`, which keeps demo and development agents responsive with one poll per minute. The eye button in the top toolbar hides demo-sensitive values such as accounts, projects, URLs, local paths, emails, auth codes, and deployment logs without changing the stored configuration or API payloads.
...
The real deploy is intentionally gated until the Foundry claim is installed on the selected source. If a local debug runtime is running, Dev Board can apply the claim to that runtime immediately; for Cloud Run, Dev Board writes the claim to the source bootstrap state and the deployment passes it into the worker for the first poll. A local runtime is not required for the Cloud Run bootstrap path. Deployment can take a few minutes while Docker builds, pushes to Artifact Registry, Cloud Run creates a revision, and Cloud Scheduler is configured; the UI shows elapsed time plus the latest deployment logs. The Cloud Run smoke test checks deployment status, Scheduler, and the latest bootstrap poll instead of using the local playground chat path. In Cloud Run mode, Dev Board keeps refreshing until discovery, invite redemption, registration approval, and a worker poll have all been observed, so an early approval callback does not make the onboarding step look complete before the live worker state has caught up.
